...
#include "IUDP.h" // Include the interface
...
// add missing vsnprintf_P method
    ...
    !defined(vsnprintf_P) && !defined(ESP8266)
...
class Syslog {
private:
  IUDP *_client; // Use pointer to interface
  uint8_t _protocol;
  IPAddress _ip;
  const char *_server;
  uint16_t _port;
...
public:
  Syslog(IUDP &client, uint8_t protocol = SYSLOG_PROTO_IETF);
  Syslog(IUDP &client, const char *server, uint16_t port,
         const char *deviceHostname = SYSLOG_NILVALUE,
         const char *appName = SYSLOG_NILVALUE, uint16_t priDefault = LOG_KERN,
         uint8_t protocol = SYSLOG_PROTO_IETF);
  Syslog(IUDP &client, IPAddress ip, uint16_t port,
         const char *deviceHostname = SYSLOG_NILVALUE,
         const char *appName = SYSLOG_NILVALUE, uint16_t priDefault = LOG_KERN,
         uint8_t protocol = SYSLOG_PROTO_IETF);
